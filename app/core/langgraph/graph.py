"""This file contains the LangGraph Agent/workflow and interactions with Gemini LLM.

Note: This implementation uses Google's Gemini models instead of OpenAI.
Ensure you have the following packages installed:
- langchain-google-genai
- google-generativeai

Configuration needed in settings:
- GEMINI_API_KEY or LLM_API_KEY: Your Google AI API key
- LLM_MODEL: e.g., "gemini-1.5-pro", "gemini-1.5-flash", "gemini-1.0-pro"
"""
...
class LangGraphAgent:
    ...
    def __init__(self):
        ...
        # Use environment-specific LLM model (Gemini)
        # Use LLM_API_KEY if GEMINI_API_KEY is not set for backward compatibility
    ...
    def _get_model_kwargs(self) -> Dict[str, Any]:
        ...
        if settings.ENVIRONMENT == Environment.DEVELOPMENT:
            ...
            model_kwargs["top_k"] = 40  # Gemini-specific parameter

        # Production - use higher quality settings
        elif settings.ENVIRONMENT == Environment.PRODUCTION:
            model_kwargs["top_p"] = 0.95
            model_kwargs["top_k"] = 40  # Gemini-specific parameter
            # Note: Gemini doesn't have presence_penalty or frequency_penalty
            # Instead, you can use candidate_count for multiple responses
    ...
    async def _chat(self, state: GraphState) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                with llm_inference_duration_seconds.labels(model=self.llm.model).time():  # Changed from model_name to model
            ...
            except (google_exceptions.GoogleAPIError, google_exceptions.RetryError, Exception) as e:
                logger.error(
                    "llm_call_failed",
                ...
                if settings.ENVIRONMENT == Environment.PRODUCTION and attempt == max_retries - 2:
                    fallback_model = "gemini-1.5-flash"  # Use flash as fallback (it's faster and more reliable)
                    logger.warning(
                        "using_fallback_model", model=fallback_model, environment=settings.ENVIRONMENT.value
                    )
                    self.llm.model = fallback_model  # Changed from model_name to model
    ...
    def __process_messages(self, messages: list[BaseMessage]) -> list[Message]:
        """Process messages to convert them to the expected format.

        Args:
            messages: List of BaseMessage objects from LangChain

        Returns:
            List of Message objects in the expected format
        """
        # Convert BaseMessages to a format compatible with our Message schema
        processed_messages = []
        for message in messages:
            if isinstance(message, HumanMessage):
                ...
                # Skip tool messages in the output
                continue
            elif hasattr(message, "role") and hasattr(message, "content"):
                # For any other message types with role and content
